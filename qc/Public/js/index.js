...
var banner1=new banner({
	...
	btn:".bannerNav",
	btnUrl:"Public/images/bannerBtn.jpg",
	btnUrlOn:"Public/images/bannerBtnOn.jpg"
});
//白点被点击
...
//	var scrolls={};
//	var scrollname=['.about','.work','.say','.advantage','.team','.service','.news'];
...
(function(){
	...
	setTimeout(function(){
		$('.mapul .ul').hide();
		$('.mapul .ul').eq(0).show();
		$('.mapul .ul').css('opacity',1);
	},300);
	$('.maptitle li').bind('click',tab);
	function tab(){
		$(".maptitle li").removeClass('on');
		$(this).addClass('on');
		$('.mapul .ul').hide();
		$('.mapul .ul').eq($(this).index()).show();
	}
})();
(function(){
	var screenA=$('.banner').height();
	var screenB=$('.conA').outerHeight();
	var screenC=$('.conB').outerHeight();
	var screenD=$('.conC').outerHeight();
	var screenE=$('.conD').outerHeight();
	var screenF=$('.work').outerHeight();
	var screenG=$('.customer').outerHeight();
	var screenH=$('.news').outerHeight();
	var screenI=$('.contact').outerHeight();
	var screenArr=[401,599,599,599,599,618,554,600,647];
	$(window).scroll(function(){
		if($(window).scrollTop()<screenA){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(0).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(1).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(2).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(3).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD+screenE)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(4).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD+screenE+screenF)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(5).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD+screenE+screenF+screenG)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(6).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD+screenE+screenF+screenG+screenH)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(7).addClass('on');
		}else if($(window).scrollTop()<(screenA+screenB+screenC+screenD+screenE+screenF+screenG+screenH+screenI)){
			$('.fixedNav a').removeClass('on');
			$('.fixedNav a').eq(8).addClass('on');
		}
	});
	$('.fixedNav a').click(function(){
		$('.fixedNav a').removeClass('on');
		$('.fixedNav a').eq($(this).index()).addClass('on');
		if($(this).index()==0){
			document.body.scrollTop=0;
		    document.documentElement.scrollTop=0;
		}else if($(this).index()==1){
			document.body.scrollTop=screenA;
		    document.documentElement.scrollTop=screenA;
		}else if($(this).index()==2){
			document.body.scrollTop=screenA+screenB;
		    document.documentElement.scrollTop=screenA+screenB;
		}else if($(this).index()==3){
			document.body.scrollTop=screenA+screenB+screenC;
		    document.documentElement.scrollTop=screenA+screenB+screenC;
		}else if($(this).index()==4){
			document.body.scrollTop=screenA+screenB+screenC+screenD;
		    document.documentElement.scrollTop=screenA+screenB+screenC+screenD;
		}else if($(this).index()==5){
			document.body.scrollTop=screenA+screenB+screenC+screenD+screenE;
		    document.documentElement.scrollTop=screenA+screenB+screenC+screenD+screenE;
		}else if($(this).index()==6){
			document.body.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF;
		    document.documentElement.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF;
		}else if($(this).index()==7){
			document.body.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF+screenG;
		    document.documentElement.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF+screenG;
		}else if($(this).index()==8){
			document.body.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF+screenG+screenH;
		    document.documentElement.scrollTop=screenA+screenB+screenC+screenD+screenE+screenF+screenG+screenH;
		}
	});
})();
